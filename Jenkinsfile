pipeline{
    ...
    stages{
        stage("Git Checkout"){
            steps{
                git credentialsId: 'github-creds', url: 'https://github.com/MS-31/javawebapplication.git'
            }
        }
	...
	stage("Upload War To Nexus"){
        ...
        stage("Deploy to Tomcat Server"){
		    ...
		    mv target/*.war target/javawebapplication.war
                    scp -o StrictHostKeyChecking=no target/javawebapplication.war  ec2-user@172.31.86.187:/opt/tomcat8/webapps/
                    ssh ec2-user@172.31.86.187 /opt/tomcat8/bin/shutdown.sh
                    ssh ec2-user@172.31.86.187 /opt/tomcat8/bin/startup.sh
    ...
    post {
	  always {
	    ...
	    success {
		mail to: 'surapaneni.mounika@gmail.com',
